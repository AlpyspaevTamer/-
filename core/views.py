...
from .forms import TestForm # Убедитесь, что у вас есть форма для теста
from django.shortcuts import render, get_object_or_404, redirect
from django.contrib.auth.decorators import login_required
from django.forms import inlineformset_factory
from .models import Test, Question, Answer
from .forms import TestForm, AnswerFormSet # Убедитесь, что AnswerFormSet импортирован
...
class TestListView(ListView):
    ...
    def get_queryset(self):
        queryset = Test.objects.annotate(questions_count=models.Count('questions'))
        if not self.request.user.is_staff:
            queryset = queryset.filter(Q(group=self.request.user.group) | Q(group__isnull=True))
...
class TestUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Настройка формсета для вопросов:
        # 1. Поля: только 'text' (убрали 'image')
        # 2. Ограничения: min_num=2 (минимум 2 вопроса), validate_min=True
        # 3. extra=0: не добавляет пустые формы автоматически
        QuestionVariantFormSet = inlineformset_factory(
            Test, 
            Question, 
            fields=('text',), 
            extra=0,
        ...
        # Убрали request.FILES, так как картинок в вопросах больше нет.
        context['variant_formset'] = QuestionVariantFormSet(
    ...
    def form_valid(self, form):
        ...
        # Валидируем форму теста и формсет вопросов
...
def add_question(request, test_id):
    ...
    Answer, 
    form=AnswerForm, 
    extra=3,       # Начнем с 1 поля (или 2, как удобнее)
    can_delete=True # Позволит удалять лишние
)
    if request.method == 'POST':
        question_form = QuestionForm(request.POST, request.FILES)
        # 1. Добавляем prefix='answers'
        ...
        if question_form.is_valid() and formset.is_valid():
            ...
            # 2. Используем formset.save() для сохранения связанных объектов
            answers = formset.save(commit=False)
            for answer in answers:
                answer.question = question
                answer.save()
            
            # 3. Валидация правильных ответов (можно перенести в clean() формсета, 
            # но оставим здесь, раз у вас это есть)
            correct_answers = sum(1 for form in formset if form.cleaned_data.get('is_correct'))
            
            if correct_answers == 0:
                question.delete()
                messages.error(request, 'Необходимо выбрать хотя бы один правильный ответ!')
                # Возвращаем с тем же префиксом
                return render(request, 'test/add_question.html', {'test': test, 'question_form': question_form, 'formset': formset})
            
            messages.success(request, 'Вопрос успешно добавлен!')
            return redirect('test_detail', pk=test.id) if 'finish' in request.POST else redirect('add_question', test_id=test.id)
        else:
            messages.error(request, 'Исправьте ошибки в форме')
            
    else:
        question_form = QuestionForm()
        # 4. Добавляем prefix='answers' и при GET
        formset = AnswerFormSet(queryset=Answer.objects.none(), prefix='answers')
...
def test_detail(request, pk):
    ...
    if is_teacher(request.user):
        questions = test.questions.prefetch_related('answers').annotate(correct_count=models.Count('answers', filter=models.Q(answers__is_correct=True)))
        return render(request, 'test/test_detail.html', {'test': test, 'questions': questions, 'is_teacher': True})
...
def select_variant(request, test_id):
    ...
    if request.method == 'POST':
        ...
        if variant_id: return redirect('take_test_variant', pk=test_id, variant_id=variant_id)
        messages.error(request, 'Выберите вариант теста')
    return render(request, 'test/select_variant.html', {'test': test, 'variants': test.variants.all()})
...
def take_test(request, pk):
    ...
    test = get_object_or_404(Test, pk=pk)
    if not request.user.is_staff and test.group is not None and test.group != request.user.group:
        raise PermissionDenied("Этот тест предназначен для другой группы.")
    ...
    if request.method == 'POST':
        ...
        for question in test.questions.all():
            ...
            if answer_id:
                ...
                except Answer.DoesNotExist: continue
        
        TestResult.objects.create(user=request.user, test=test, score=score, max_score=total)
        
        messages.success(request, f'Результат: {score}/{total}')
        return redirect('test_results', pk=test.id, score=score)
    return render(request, 'test/take_test.html', {'test': test, 'questions': test.questions.all()})

@login_required
@user_passes_test(is_teacher)
def view_test_results(request, pk):
    test = get_object_or_404(Test, pk=pk)
    results = test.results.all().select_related('user')
    return render(request, 'test/admin_results.html', {'test': test, 'results': results})
...
def test_results(request, pk, score):
    ...
    return render(request, 'test/results.html', {'test': test, 'score': score, 'total': total, 'percentage': percentage})
...
def test_questions(request, pk):
    ...
    return render(request, 'test/questions.html', {'test': test, 'questions': questions})
...
from django.forms import inlineformset_factory
from .models import Test, Question, Answer
from .forms import TestForm, AnswerForm # Убедитесь, что у вас есть простая AnswerForm

# Фабрику создаем здесь, чтобы избежать конфликтов
AnswerFormSet = inlineformset_factory(
    Question, Answer, fields=['text', 'is_correct'], extra=0, can_delete=False
)
...
def edit_test(request, pk):
    ...
    # 1. Формы для самого теста и вопросов
    test_form = TestForm(request.POST or None, instance=test)
    QuestionFormSet = inlineformset_factory(Test, Question, fields=['text', 'points'], extra=0)
    question_formset = QuestionFormSet(request.POST or None, instance=test)
    
    # 2. Инициализируем AnswerFormSet фабрику (лучше один раз здесь или в forms.py)
    AnswerFormSet = inlineformset_factory(
        Question, Answer, fields=['text', 'is_correct'], extra=0, can_delete=False
    )
    ...
    if request.method == 'POST':
        if test_form.is_valid() and question_formset.is_valid():
            ...
            # ВАЖНО: После сохранения вопросов, у них появляются ID (если они были новыми)
            # Теперь проходимся по вопросам и сохраняем ответы
            for q_form in question_formset:
                # Берем объект вопроса (q_form.instance)
                question = q_form.instance
                # Создаем формсет ответов с уникальным префиксом для этого вопроса
                ans_formset = AnswerFormSet(
                    request.POST, 
                    instance=question, 
                    prefix=f"ans_{question.id}"
                )
        ...
        # В GET запросе мы просто привязываем пустые/существующие формы к вопросам
        for q_form in question_formset:
            q_form.nested_answer_formset = AnswerFormSet(
                instance=q_form.instance, 
                prefix=f"ans_{q_form.instance.id}"
            )
...
def get_points_state(request):
    try:
        ...
        # Рассчитываем оставшееся время кулдауна в секундах
        ...
        if user_points.last_slow_click:
            ...
            if elapsed < user_points.SLOW_CLICK_COOLDOWN:
                ...
                cooldown_end = user_points.last_slow_click + user_points.SLOW_CLICK_COOLDOWN
                cooldown_end_timestamp = int(cooldown_end.timestamp() * 1000)
        
        # Рассчитываем время следующего сброса лимита для конкретного пользователя
        now = timezone.now()
        if user_points.first_click_today:
            # Если был первый клик - сброс через 24 часа после него
            next_reset = user_points.first_click_today + timedelta(days=1)
        else:
            # Если кликов сегодня не было - сброс в 00:00 следующего дня
            next_reset = (now + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        
        next_reset_timestamp = int(next_reset.timestamp() * 1000)
...
class theory(TemplateView):
    template_name = 'mathsolver/theory.html'

class drobi(TemplateView):
    template_name = 'mathsolver/drobi.html'
...
def manage_groups(request):
    ...
    if request.method == 'POST':
        ...
        if name and code:
            StudyGroup.objects.create(name=name, group_code=code)
...
def assign_user_group(request, user_id):
    ...
    if request.method == 'POST':
        form = AssignGroupForm(request.POST, instance=user)
        if form.is_valid():
            form.save()
            return redirect('admin_user_list')
    else:
        form = AssignGroupForm(instance=user)
    return render(request, 'admin/assign_group.html', {'form': form, 'user_obj': user})


@staff_member_required
def manage_groups_page(request):
    groups = StudyGroup.objects.all()
    if request.method == 'POST':
        ...
        if name and code:
            StudyGroup.objects.create(name=name, group_code=code)
            return redirect('manage_groups') # Перезагружаем страницу
    return render(request, 'admin/manage_groups.html', {'groups': groups})
...
def delete_user(request, pk):
    # Добавьте проверку POST, чтобы нельзя было удалить пользователя просто переходом по ссылке
    if request.method != 'POST':
        return redirect('admin_user_list') 
    ...
    if user_to_delete == request.user:
        ...
        messages.success(request, f"Пользователь {user_to_delete.username} удален.")
        
    return redirect('admin_user_list') # Имя теперь совпадает с тем, что в urls.py
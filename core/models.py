...
class StudyGroup(models.Model):
    ...
    # unique=True гарантирует, что нельзя создать две группы с одинаковым кодом
    group_code = models.CharField("Код группы (ID)", max_length=20, unique=True)
...
class User(AbstractUser):
    ...
    group = models.ForeignKey(StudyGroup, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Учебная группа")
...
class Test(models.Model):
    ...
    group = models.ForeignKey(
        ...
        help_text="Оставьте пустым, если тест доступен всем"
    )
...
class TestResult(models.Model):
    # Добавили related_name для удобного доступа к данным
    ...
    test = models.ForeignKey(
        'Test', # Используем строку, если модель Test определена в этом же файле ниже
        on_delete=models.CASCADE, 
    ...
    # Полезное поле для хранения структуры ответа (понадобится, если захотите расширить функционал)
    details = models.JSONField(verbose_name="Детали результатов", default=dict, blank=True)
